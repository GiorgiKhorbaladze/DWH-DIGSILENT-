...
def test_export_omits_profile_id_for_non_bindable_default_assets() -> None:
    html = _html()
    assets = _extract_state_assets(html)
    bindable = {"wind", "solar", "hydro_ror"}
    for asset in assets:
        if asset["type"] in bindable:
            assert "profile_id" in asset
        else:
            assert "profile_id" not in asset
...
def test_validation_rules_are_present() -> None:
    ...
    assert "!isProfileBindableAsset(asset.type)" in html
